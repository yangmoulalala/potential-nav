...
: Node("fake_vel_transform", options)
{
  ...
  this->declare_parameter<bool>("synchronize_cmd_vel_with_local_plan", false);
  ...
  this->get_parameter("synchronize_cmd_vel_with_local_plan", synchronize_cmd_vel_with_local_plan_);
  ...
  const nav_msgs::msg::Path::ConstSharedPtr & /*local_plan_msg*/)
{
  ...
  if (!synchronize_cmd_vel_with_local_plan_ || !latest_cmd_vel_) {
    return;
  }

  auto current_cmd_vel = latest_cmd_vel_;
  latest_cmd_vel_.reset();
  auto aft_tf_vel = transformVelocity(current_cmd_vel, current_robot_base_angle_);
  cmd_vel_chassis_pub_->publish(aft_tf_vel);
}
